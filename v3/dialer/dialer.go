...
func (d *Dialer) Connect(ctx context.Context) (*ggrpc.ClientConn, error) {
	...
	// Read jwt secret specific to the endpoint
	jwtSecret, err := d.dm.config.ReadJwt(ctx, d.endpointName)
	if err != nil {
		return nil, fmt.Errorf("jwt secret for %s not found, error: %s", d.endpointName, err.Error())
	}

	middlewares := []middleware.Middleware{
		u_auth.Client(jwtSecret, d.dm.config.GetAppName(), d.endpointName),
		metadata.Client(),
	}
	...
	conn, err := grpc.DialInsecure(
		...
		grpc.WithDiscovery(d.dm.config.GetRegistry()),
